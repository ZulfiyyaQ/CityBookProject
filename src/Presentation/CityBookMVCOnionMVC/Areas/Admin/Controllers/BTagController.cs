...
namespace CityBookMVCOnionMVC.Areas.Admin.Controllers
{
    ...
    {
        private readonly IBTagService _service;

        public BTagController(IBTagService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create(CreateBTagVM create)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> Update(int id, UpdateBTagVM update)
        {
        ...
        {
            GetBTagVM get = await _service.GetByIdAsync(id);
